...
//        model.addAttribute("user", user);
        CreateInvoiceController.setUser(user);
        CreateInvoiceController.setOperator(operator);
    ...
    public String createInvoice(@ModelAttribute CreateInvoice invoice, Model model) {
        ...
        final Station station = new Station(new Address(invoice.getRegion(), invoice.getCity(), String.valueOf(invoice.getStationNumber())), (short) invoice.getStationNumber());
...
//                .id(invoice.hashCode())
                ...
                .build();
        final Package aPackage = new Package(invoice.hashCode(), invoice.getWeight(), invoice.getVolume(), invoice.getInsurance());
